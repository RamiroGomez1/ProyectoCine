import { a as __awaiter, f as _objectSpread2$3, l as __rest, t as _asyncToGenerator } from "./asyncToGenerator-BdaQx21n.js";
...
*/
function fetchWithRetry(_x, _x2, _x3, _x4) {
	return _fetchWithRetry.apply(this, arguments);
}
function _fetchWithRetry() {
	_fetchWithRetry = _asyncToGenerator(function* (fetchImpl, url, request, retryEnabled) {
		let attemptCount = 0;
		while (true) {
			...
			try {
				...
				if (retryEnabled && attemptCount < DEFAULT_MAX_RETRIES) {
					...
					attemptCount++;
					yield sleep$1(delay, request.signal);
					continue;
				}
				throw fetchError;
			}
	...
	*/
	...
	then(onfulfilled, onrejected) {
		...
		let res = function() {
			...
			return function executeWithRetry() {
	...
	*/
	constructor(endPoint, opts = {}) {
		...
		this.reconnectTimer = new Timer(() => {
			...
			this.teardown(_asyncToGenerator(function* () {
...
var RealtimeChannel = class RealtimeChannel {
	...
	*/
	subscribe(callback, timeout = this.timeout) {
		...
		if (this.channelAdapter.isClosed()) {
			...
			this.channelAdapter.subscribe(joinTimeout).receive("ok", function() {
				var _ref = _asyncToGenerator(function* ({ postgres_changes }) {
					if (!_this.socket._isManualToken()) _this.socket.setAuth();
					if (postgres_changes === void 0) {
						callback === null || callback === void 0 || callback(REALTIME_SUBSCRIBE_STATES.SUBSCRIBED);
						return;
					}
					_this._updatePostgresBindings(postgres_changes, callback);
				});
				return function(_x) {
					return _ref.apply(this, arguments);
				};
	...
	*/
	httpSend(_x3, _x4) {
		var _this4 = this;
		return _asyncToGenerator(function* (event, payload, opts = {}) {
			var _a;
			...
			if (_this4.socket.accessTokenValue) headers["Authorization"] = `Bearer ${_this4.socket.accessTokenValue}`;
			const url = new URL(_this4.broadcastEndpointURL);
			url.pathname += `/${encodeURIComponent(_this4.subTopic)}/events/${encodeURIComponent(event)}`;
			if (_this4.private) url.searchParams.set("private", "true");
			const options = {
				method: "POST",
				headers,
				body: isBinary ? payload : JSON.stringify(payload)
			};
			const response = yield _this4._fetchWithTimeout(url.toString(), options, (_a = opts.timeout) !== null && _a !== void 0 ? _a : _this4.timeout);
	...
	*/
	send(_x5) {
		...
		return _asyncToGenerator(function* (args, opts = {}) {
			var _a, _b;
			if (!_this5.channelAdapter.canPush() && args.type === "broadcast") {
				...
				if (_this5.socket.hasLogger()) _this5.socket.log("channel", fallbackWarning);
				else console.warn(fallbackWarning);
				const { event, payload: endpoint_payload } = args;
				const headers = {
					apikey: _this5.socket.apiKey ? _this5.socket.apiKey : "",
					"Content-Type": "application/json"
				};
				if (_this5.socket.accessTokenValue) headers["Authorization"] = `Bearer ${_this5.socket.accessTokenValue}`;
				const options = {
					method: "POST",
					headers,
					body: JSON.stringify({ messages: [{
						topic: _this5.subTopic,
						event,
						payload: endpoint_payload,
						private: _this5.private
					}] })
				};
				try {
					const response = yield _this5._fetchWithTimeout(_this5.broadcastEndpointURL, options, (_a = opts.timeout) !== null && _a !== void 0 ? _a : _this5.timeout);
				...
				var _a, _b, _c;
				const push = _this5.channelAdapter.push(args.type, args, opts.timeout || _this5.timeout);
				if (args.type === "broadcast" && !((_c = (_b = (_a = _this5.params) === null || _a === void 0 ? void 0 : _a.config) === null || _b === void 0 ? void 0 : _b.broadcast) === null || _c === void 0 ? void 0 : _c.ack)) resolve("ok");
	...
	*/
	unsubscribe() {
		var _this6 = this;
		return _asyncToGenerator(function* (timeout = this.timeout) {
			return new Promise((resolve) => {
				_this6.channelAdapter.unsubscribe(timeout).receive("ok", () => resolve("ok")).receive("timeout", () => resolve("timed out")).receive("error", () => resolve("error"));
			});
		}).apply(this, arguments);
	}
	/**
	...
	*/
	...
	_fetchWithTimeout(url, options, timeout) {
		...
		return _asyncToGenerator(function* () {
			...
			const response = yield _this7.socket.fetch(url, Object.assign(Object.assign({}, options), { signal: controller.signal }));
			clearTimeout(id);
			return response;
		})();
	}
...
var RealtimeClient = class {
	...
	*/
	disconnect(code, reason) {
		var _this = this;
		return _asyncToGenerator(function* () {
			_this._cancelPendingDisconnect();
			if (_this.isDisconnecting()) return "ok";
			return yield _this.socketAdapter.disconnect(() => {
				clearInterval(_this._workerHeartbeatTimer);
	...
	*/
	removeAllChannels() {
		var _this2 = this;
		return _asyncToGenerator(function* () {
			const promises = _this2.channels.map(function() {
				var _ref = _asyncToGenerator(function* (channel) {
					const result = yield channel.unsubscribe();
					channel.teardown();
					return result;
				});
				return function(_x) {
	...
	*/
	setAuth(token = null) {
		var _this3 = this;
		return _asyncToGenerator(function* () {
			const authGeneration = ++_this3._authGeneration;
			const authPromise = _this3._performAuth(token, authGeneration);
			if (authGeneration === _this3._authGeneration) _this3._authPromise = authPromise;
			try {
				yield authPromise;
			} finally {
				if (_this3._authPromise === authPromise) _this3._authPromise = null;
			}
		})();
	}
	/**
	...
	*/
	sendHeartbeat() {
		var _this4 = this;
		return _asyncToGenerator(function* () {
			_this4.socketAdapter.sendHeartbeat();
		})();
	}
	/**
	* Sets a callback that receives lifecycle events for internal heartbeat messages.
	...
	_performAuth(token, authGeneration) {
		...
		return _asyncToGenerator(function* () {
			...
			if (token) {
				tokenToSend = token;
				isManualToken = true;
			} else if (_this5.accessToken) try {
				tokenToSend = yield _this5.accessToken();
			} catch (e) {
				_this5.log("error", "Error fetching access token from callback", e);
				tokenToSend = _this5.accessTokenValue;
			}
			else tokenToSend = _this5.accessTokenValue;
			if (authGeneration !== _this5._authGeneration) return;
			if (_this5.accessToken) _this5._manuallySetToken = false;
			else if (isManualToken) _this5._manuallySetToken = true;
	...
	*/
	_waitForAuthIfNeeded() {
		var _this6 = this;
		return _asyncToGenerator(function* () {
			if (_this6._authPromise) yield _this6._authPromise;
		})();
	}
	/**
	...
	*/
	...
	_reconnectAuth() {
		...
		return _asyncToGenerator(function* () {
			...
			if (!_this7.isConnected()) _this7.connect();
		})();
	}
};
//#endregion
...
var IcebergError = class extends Error {
	constructor(message, opts) {
		...
		this.isCommitStateUnknown = opts.icebergType === "CommitStateUnknownException" || [
			...
			502,
			504
		].includes(opts.status) && ((_opts$icebergType = opts.icebergType) === null || _opts$icebergType === void 0 ? void 0 : _opts$icebergType.includes("CommitState")) === true;
	}
	/**
	...
	return { request({ method, path, query, body, headers }) {
		return _asyncToGenerator(function* () {
			...
			const res = yield fetchFn(url, {
				method,
				headers: _objectSpread2$3(_objectSpread2$3(_objectSpread2$3({}, body ? { "Content-Type": "application/json" } : {}), authHeaders), headers),
			...
			if (!res.ok) {
				...
				throw new IcebergError((_errorDetail$message = errorDetail === null || errorDetail === void 0 ? void 0 : errorDetail.message) !== null && _errorDetail$message !== void 0 ? _errorDetail$message : `Request failed with status ${res.status}`, {
					status: res.status,
...
var NamespaceOperations = class {
	...
	listNamespaces(parent) {
		...
		return _asyncToGenerator(function* () {
			...
			return (yield _this.client.request({
				method: "GET",
				path: `${_this.prefix}/namespaces`,
				query
	...
	createNamespace(id, metadata) {
		...
		return _asyncToGenerator(function* () {
			...
			return (yield _this2.client.request({
				method: "POST",
				path: `${_this2.prefix}/namespaces`,
				body: request
			})).data;
		})();
	}
	dropNamespace(id) {
		var _this3 = this;
		return _asyncToGenerator(function* () {
			yield _this3.client.request({
				method: "DELETE",
				path: `${_this3.prefix}/namespaces/${namespaceToPath(id.namespace)}`
			});
		})();
	}
	loadNamespaceMetadata(id) {
		var _this4 = this;
		return _asyncToGenerator(function* () {
			return { properties: (yield _this4.client.request({
				method: "GET",
				path: `${_this4.prefix}/namespaces/${namespaceToPath(id.namespace)}`
			})).data.properties };
		})();
	}
	namespaceExists(id) {
		...
		return _asyncToGenerator(function* () {
			try {
				yield _this5.client.request({
					method: "HEAD",
					path: `${_this5.prefix}/namespaces/${namespaceToPath(id.namespace)}`
				});
				return true;
			} catch (error) {
	...
	createNamespaceIfNotExists(id, metadata) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this6.createNamespace(id, metadata);
			} catch (error) {
...
var TableOperations = class {
	...
	listTables(namespace) {
		var _this7 = this;
		return _asyncToGenerator(function* () {
			return (yield _this7.client.request({
				method: "GET",
				path: `${_this7.prefix}/namespaces/${namespaceToPath2(namespace.namespace)}/tables`
			})).data.identifiers;
		})();
	}
	createTable(namespace, request) {
		...
		return _asyncToGenerator(function* () {
			const headers = {};
			if (_this8.accessDelegation) headers["X-Iceberg-Access-Delegation"] = _this8.accessDelegation;
			return (yield _this8.client.request({
				method: "POST",
				path: `${_this8.prefix}/namespaces/${namespaceToPath2(namespace.namespace)}/tables`,
				body: request,
				headers
			})).data.metadata;
		})();
	...
	updateTable(id, request) {
		var _this9 = this;
		return _asyncToGenerator(function* () {
			const response = yield _this9.client.request({
				method: "POST",
				path: `${_this9.prefix}/namespaces/${namespaceToPath2(id.namespace)}/tables/${id.name}`,
				body: request
			});
			return {
	...
	dropTable(id, options) {
		...
		return _asyncToGenerator(function* () {
			var _options$purge;
			yield _this10.client.request({
				method: "DELETE",
				path: `${_this10.prefix}/namespaces/${namespaceToPath2(id.namespace)}/tables/${id.name}`,
				query: { purgeRequested: String((_options$purge = options === null || options === void 0 ? void 0 : options.purge) !== null && _options$purge !== void 0 ? _options$purge : false) }
			});
		})();
	}
	loadTable(id) {
		...
		return _asyncToGenerator(function* () {
			const headers = {};
			if (_this11.accessDelegation) headers["X-Iceberg-Access-Delegation"] = _this11.accessDelegation;
			return (yield _this11.client.request({
				method: "GET",
				path: `${_this11.prefix}/namespaces/${namespaceToPath2(id.namespace)}/tables/${id.name}`,
				headers
			})).data.metadata;
		})();
	}
	tableExists(id) {
		...
		return _asyncToGenerator(function* () {
			const headers = {};
			if (_this12.accessDelegation) headers["X-Iceberg-Access-Delegation"] = _this12.accessDelegation;
			try {
				yield _this12.client.request({
					method: "HEAD",
					path: `${_this12.prefix}/namespaces/${namespaceToPath2(id.namespace)}/tables/${id.name}`,
					headers
				});
				return true;
			} catch (error) {
	...
	createTableIfNotExists(namespace, request) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this13.createTable(namespace, request);
			} catch (error) {
				if (error instanceof IcebergError && error.status === 409) return yield _this13.loadTable({
...
var IcebergRestCatalog = class {
	...
	*/
	listNamespaces(parent) {
		var _this14 = this;
		return _asyncToGenerator(function* () {
			return _this14.namespaceOps.listNamespaces(parent);
		})();
	}
	/**
	...
	*/
	createNamespace(id, metadata) {
		var _this15 = this;
		return _asyncToGenerator(function* () {
			return _this15.namespaceOps.createNamespace(id, metadata);
		})();
	}
	/**
	...
	*/
	dropNamespace(id) {
		var _this16 = this;
		return _asyncToGenerator(function* () {
			yield _this16.namespaceOps.dropNamespace(id);
		})();
	}
	/**
	...
	*/
	loadNamespaceMetadata(id) {
		var _this17 = this;
		return _asyncToGenerator(function* () {
			return _this17.namespaceOps.loadNamespaceMetadata(id);
		})();
	}
	/**
	...
	*/
	listTables(namespace) {
		var _this18 = this;
		return _asyncToGenerator(function* () {
			return _this18.tableOps.listTables(namespace);
		})();
	}
	/**
	...
	*/
	createTable(namespace, request) {
		var _this19 = this;
		return _asyncToGenerator(function* () {
			return _this19.tableOps.createTable(namespace, request);
		})();
	}
	/**
	...
	*/
	updateTable(id, request) {
		var _this20 = this;
		return _asyncToGenerator(function* () {
			return _this20.tableOps.updateTable(id, request);
		})();
	}
	/**
	...
	*/
	dropTable(id, options) {
		var _this21 = this;
		return _asyncToGenerator(function* () {
			yield _this21.tableOps.dropTable(id, options);
		})();
	}
	/**
	...
	*/
	loadTable(id) {
		var _this22 = this;
		return _asyncToGenerator(function* () {
			return _this22.tableOps.loadTable(id);
		})();
	}
	/**
	...
	*/
	namespaceExists(id) {
		var _this23 = this;
		return _asyncToGenerator(function* () {
			return _this23.namespaceOps.namespaceExists(id);
		})();
	}
	/**
	...
	*/
	tableExists(id) {
		var _this24 = this;
		return _asyncToGenerator(function* () {
			return _this24.tableOps.tableExists(id);
		})();
	}
	/**
	...
	*/
	createNamespaceIfNotExists(id, metadata) {
		var _this25 = this;
		return _asyncToGenerator(function* () {
			return _this25.namespaceOps.createNamespaceIfNotExists(id, metadata);
		})();
	}
	/**
	...
	*/
	createTableIfNotExists(namespace, request) {
		var _this26 = this;
		return _asyncToGenerator(function* () {
			return _this26.tableOps.createTableIfNotExists(namespace, request);
		})();
	}
};
//#endregion
...
*/
var handleError$1 = function() {
	var _ref = _asyncToGenerator(function* (error, reject, options, namespace) {
	...
	return function handleError(_x, _x2, _x3, _x4) {
		return _ref.apply(this, arguments);
	};
}();
/**
...
*/
function _handleRequest$1(_x5, _x6, _x7, _x8, _x9, _x10, _x11) {
	return _handleRequest2$1.apply(this, arguments);
}
function _handleRequest2$1() {
	_handleRequest2$1 = _asyncToGenerator(function* (fetcher, method, url, options, parameters, body, namespace) {
...
function createFetchApi(namespace = "storage") {
	return {
		get: function() {
			var _ref2 = _asyncToGenerator(function* (fetcher, url, options, parameters) {
		...
		post: function() {
			var _ref3 = _asyncToGenerator(function* (fetcher, url, body, options, parameters) {
				return _handleRequest$1(fetcher, "POST", url, options, parameters, body, namespace);
			});
			return function post(_x16, _x17, _x18, _x19, _x20) {
				return _ref3.apply(this, arguments);
			};
		}(),
		put: function() {
			var _ref4 = _asyncToGenerator(function* (fetcher, url, body, options, parameters) {
				return _handleRequest$1(fetcher, "PUT", url, options, parameters, body, namespace);
			});
			return function put(_x21, _x22, _x23, _x24, _x25) {
				return _ref4.apply(this, arguments);
			};
		}(),
		head: function() {
			var _ref5 = _asyncToGenerator(function* (fetcher, url, options, parameters) {
			...
			return function head(_x26, _x27, _x28, _x29) {
				return _ref5.apply(this, arguments);
			};
		}(),
		remove: function() {
			var _ref6 = _asyncToGenerator(function* (fetcher, url, body, options, parameters) {
				return _handleRequest$1(fetcher, "DELETE", url, options, parameters, body, namespace);
			});
			return function remove(_x30, _x31, _x32, _x33, _x34) {
...
var StorageFileApi = class extends BaseApiClient {
	...
	*/
	uploadOrUpdate(method, path, fileBody, fileOptions) {
		...
		return _asyncToGenerator(function* () {
			var _this = _this19;
			return _this.handleOperation(_asyncToGenerator(function* () {
				let body;
				...
				const data = yield (method == "PUT" ? put : post)(_this.fetch, `${_this.url}/object/${_path}`, body, _objectSpread2$1({ headers }, (options === null || options === void 0 ? void 0 : options.duplex) ? { duplex: options.duplex } : {}));
				return {
					path: cleanPath,
					id: data.Id,
					fullPath: data.Key
	...
	*/
	upload(path, fileBody, fileOptions) {
		var _this20 = this;
		return _asyncToGenerator(function* () {
			return _this20.uploadOrUpdate("POST", path, fileBody, fileOptions);
		})();
	}
	/**
	...
	*/
	uploadToSignedUrl(path, token, fileBody, fileOptions) {
		...
		return _asyncToGenerator(function* () {
			...
			return _this3.handleOperation(_asyncToGenerator(function* () {
				let body;
				const options = _objectSpread2$1(_objectSpread2$1({}, DEFAULT_FILE_OPTIONS), fileOptions);
				...
				return {
					path: cleanPath,
					fullPath: (yield put(_this3.fetch, url.toString(), body, _objectSpread2$1({ headers }, (options === null || options === void 0 ? void 0 : options.duplex) ? { duplex: options.duplex } : {}))).Key
				};
			}));
		})();
	}
	...
	*/
	createSignedUploadUrl(path, options) {
		...
		return _asyncToGenerator(function* () {
			var _this4 = _this22;
			return _this4.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	update(path, fileBody, fileOptions) {
		var _this23 = this;
		return _asyncToGenerator(function* () {
			return _this23.uploadOrUpdate("PUT", path, fileBody, fileOptions);
		})();
	}
	/**
	...
	*/
	move(fromPath, toPath, options) {
		...
		return _asyncToGenerator(function* () {
			var _this6 = _this24;
			return _this6.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	copy(fromPath, toPath, options) {
		...
		return _asyncToGenerator(function* () {
			var _this7 = _this25;
			return _this7.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	createSignedUrl(path, expiresIn, options) {
		...
		return _asyncToGenerator(function* () {
			var _this8 = _this26;
			return _this8.handleOperation(_asyncToGenerator(function* () {
				let _path = _this8._getFinalPath(path);
				const hasTransform = typeof (options === null || options === void 0 ? void 0 : options.transform) === "object" && options.transform !== null && Object.keys(options.transform).length > 0;
				let data = yield post(_this8.fetch, `${_this8.url}/object/sign/${_path}`, _objectSpread2$1(_objectSpread2$1({ expiresIn }, hasTransform ? { transform: options.transform } : {}), (options === null || options === void 0 ? void 0 : options.versionId) != null ? { versionId: options.versionId } : {}), { headers: _this8.headers });
	...
	*/
	createSignedUrls(paths, expiresIn, options) {
		...
		return _asyncToGenerator(function* () {
			var _this9 = _this27;
			return _this9.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	info(path, options) {
		...
		return _asyncToGenerator(function* () {
			...
			if ((options === null || options === void 0 ? void 0 : options.versionId) != null) query.set("versionId", String(options.versionId));
			const queryString = query.toString();
			return _this10.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	remove(paths) {
		...
		return _asyncToGenerator(function* () {
			var _this12 = _this30;
			return _this12.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	purgeCache(path, options, parameters) {
		...
		return _asyncToGenerator(function* () {
			var _this13 = _this31;
			return _this13.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	list(path, options, parameters) {
		...
		return _asyncToGenerator(function* () {
			var _this14 = _this32;
			return _this14.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	listV2(options, parameters) {
		...
		return _asyncToGenerator(function* () {
			var _this15 = _this33;
			return _this15.handleOperation(_asyncToGenerator(function* () {
...
var StorageBucketApi = class extends BaseApiClient {
	...
	*/
	getBucket(id) {
		...
		return _asyncToGenerator(function* () {
			var _this2 = _this35;
			return _this2.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	createBucket(_x35) {
		var _this36 = this;
		return _asyncToGenerator(function* (id, options = { public: false }) {
			var _this3 = _this36;
			return _this3.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	emptyBucket(id) {
		...
		return _asyncToGenerator(function* () {
			var _this5 = _this38;
			return _this5.handleOperation(_asyncToGenerator(function* () {
	...
	*/
	getBucketLifecycle(id) {
		...
		return _asyncToGenerator(function* () {
			var _this7 = _this40;
			return _this7.handleOperation(_asyncToGenerator(function* () {
				return yield get(_this7.fetch, _this7.bucketLifecycleUrl(id), { headers: _this7.headers });
			}));
		})();
	}
	/**
	...
	*/
	updateBucketLifecycle(id, configuration) {
		...
		return _asyncToGenerator(function* () {
			var _this8 = _this41;
			return _this8.handleOperation(_asyncToGenerator(function* () {
				return yield put(_this8.fetch, _this8.bucketLifecycleUrl(id), configuration, { headers: _this8.headers });
			}));
		})();
	}
	/**
	...
	*/
	deleteBucketLifecycle(id) {
		...
		return _asyncToGenerator(function* () {
			var _this9 = _this42;
			return _this9.handleOperation(_asyncToGenerator(function* () {
				return yield remove(_this9.fetch, _this9.bucketLifecycleUrl(id), {}, { headers: _this9.headers });
			}));
		})();
	}
	/**
	...
	purgeBucketCache(id, options, parameters) {
		...
		return _asyncToGenerator(function* () {
			...
			return _this10.handleOperation(_asyncToGenerator(function* () {
				...
				if (options === null || options === void 0 ? void 0 : options.transformations) query.set("transformations", "true");
				const queryString = query.toString();
				return yield remove(_this10.fetch, `${_this10.url}/cdn/${encodeStoragePath(id)}${queryString ? `?${queryString}` : ""}`, {}, { headers: _this10.headers }, parameters);
			}));
		})();
	}
...
var StorageVectorsClient = class extends VectorBucketApi {
	...
	*/
	createBucket(vectorBucketName) {
		var _superprop_getCreateBucket2 = () => super.createBucket, _this60 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getCreateBucket = () => _superprop_getCreateBucket2(), _this = _this60;
	...
	*/
	getBucket(vectorBucketName) {
		var _superprop_getGetBucket2 = () => super.getBucket, _this61 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getGetBucket = () => _superprop_getGetBucket2(), _this2 = _this61;
	...
	*/
	listBuckets() {
		var _superprop_getListBuckets2 = () => super.listBuckets, _this62 = this;
		return _asyncToGenerator(function* (options = {}) {
			var _superprop_getListBuckets = () => _superprop_getListBuckets2(), _this3 = _this62;
	...
	*/
	deleteBucket(vectorBucketName) {
		var _superprop_getDeleteBucket2 = () => super.deleteBucket, _this63 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getDeleteBucket = () => _superprop_getDeleteBucket2(), _this4 = _this63;
...
var VectorBucketScope = class extends VectorIndexApi {
	...
	*/
	createIndex(options) {
		var _superprop_getCreateIndex2 = () => super.createIndex, _this64 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getCreateIndex = () => _superprop_getCreateIndex2(), _this5 = _this64;
	...
	*/
	listIndexes() {
		var _superprop_getListIndexes2 = () => super.listIndexes, _this65 = this;
		return _asyncToGenerator(function* (options = {}) {
			var _superprop_getListIndexes = () => _superprop_getListIndexes2(), _this6 = _this65;
	...
	*/
	getIndex(indexName) {
		var _superprop_getGetIndex2 = () => super.getIndex, _this66 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getGetIndex = () => _superprop_getGetIndex2(), _this7 = _this66;
	...
	*/
	deleteIndex(indexName) {
		var _superprop_getDeleteIndex2 = () => super.deleteIndex, _this67 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getDeleteIndex = () => _superprop_getDeleteIndex2(), _this8 = _this67;
...
var VectorIndexScope = class extends VectorDataApi {
	...
	*/
	putVectors(options) {
		var _superprop_getPutVectors2 = () => super.putVectors, _this68 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getPutVectors = () => _superprop_getPutVectors2(), _this9 = _this68;
	...
	*/
	getVectors(options) {
		var _superprop_getGetVectors2 = () => super.getVectors, _this69 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getGetVectors = () => _superprop_getGetVectors2(), _this10 = _this69;
	...
	*/
	listVectors() {
		var _superprop_getListVectors2 = () => super.listVectors, _this70 = this;
		return _asyncToGenerator(function* (options = {}) {
			var _superprop_getListVectors = () => _superprop_getListVectors2(), _this11 = _this70;
	...
	*/
	queryVectors(options) {
		var _superprop_getQueryVectors2 = () => super.queryVectors, _this71 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getQueryVectors = () => _superprop_getQueryVectors2(), _this12 = _this71;
	...
	*/
	deleteVectors(options) {
		var _superprop_getDeleteVectors2 = () => super.deleteVectors, _this72 = this;
		return _asyncToGenerator(function* () {
			var _superprop_getDeleteVectors = () => _superprop_getDeleteVectors2(), _this13 = _this72;
...
function generatePKCEChallenge(_x10) {
...
*/
function getPKCEFlowIndex(_x11, _x12) {
	return _getPKCEFlowIndex.apply(this, arguments);
}
function _getPKCEFlowIndex() {
	_getPKCEFlowIndex = _asyncToGenerator(function* (storage, storageKey) {
...
*/
function storePKCEVerifier(_x13, _x14, _x15, _x16, _x17) {
	return _storePKCEVerifier.apply(this, arguments);
}
function _storePKCEVerifier() {
	_storePKCEVerifier = _asyncToGenerator(function* (storage, storageKey, flowId, verifier, onEvictFlow) {
...
*/
function retrievePKCEVerifier(_x18, _x19, _x20) {
	return _retrievePKCEVerifier.apply(this, arguments);
}
function _retrievePKCEVerifier() {
	_retrievePKCEVerifier = _asyncToGenerator(function* (storage, storageKey, flowId) {
		if (flowId) {
			const verifier = yield getItemAsync(storage, pkceVerifierSlotKey(storageKey, flowId));
			return {
...
*/
function removePKCEVerifier(_x21, _x22, _x23) {
	return _removePKCEVerifier.apply(this, arguments);
}
function _removePKCEVerifier() {
	_removePKCEVerifier = _asyncToGenerator(function* (storage, storageKey, flowId) {
		...
		if (remaining.length !== index.length) if (remaining.length > 0) yield setItemAsync(storage, pkceFlowIndexKey(storageKey), remaining);
		else yield removeItemAsync(storage, pkceFlowIndexKey(storageKey));
		if (slotValue != null && slotValue === (yield getItemAsync(storage, legacyKey))) yield removeItemAsync(storage, legacyKey);
	});
	return _removePKCEVerifier.apply(this, arguments);
}
/**
...
*/
function removeAllPKCEVerifiers(_x24, _x25) {
	return _removeAllPKCEVerifiers.apply(this, arguments);
}
function _removeAllPKCEVerifiers() {
	_removeAllPKCEVerifiers = _asyncToGenerator(function* (storage, storageKey) {
		const index = yield getPKCEFlowIndex(storage, storageKey);
		for (const flowId of index) yield removeItemAsync(storage, pkceVerifierSlotKey(storageKey, flowId));
...
function getCodeChallengeAndMethod(_x26, _x27) {
	return _getCodeChallengeAndMethod.apply(this, arguments);
}
function _getCodeChallengeAndMethod() {
	_getCodeChallengeAndMethod = _asyncToGenerator(function* (storage, storageKey, isPasswordRecovery = false, onEvictFlow) {
...
function _request2() {
	_request2 = _asyncToGenerator(function* (fetcher, method, url, options) {
		...
		const data = yield _handleRequest(fetcher, method, url + (Object.keys(qs).length ? "?" + new URLSearchParams(qs).toString() : ""), {
			headers,
...
function _handleRequest(_x6, _x7, _x8, _x9, _x10, _x11) {
...
function _handleRequest2() {
	_handleRequest2 = _asyncToGenerator(function* (fetcher, method, url, options, parameters, body) {
		...
		if (!result.ok) yield handleError(result);
		if (options === null || options === void 0 ? void 0 : options.noResolveJson) return result;
		try {
...
var GoTrueAdminApi = class {
	...
	*/
	...
	_listFactors(params) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				const { data, error } = yield _request(_this9.fetch, "GET", `${_this9.url}/admin/users/${params.userId}/factors`, {
					headers: _this9.headers,
					xform: (factors) => {
						return {
							data: { factors },
	...
	_deleteFactor(params) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				return {
					data: yield _request(_this10.fetch, "DELETE", `${_this10.url}/admin/users/${params.userId}/factors/${params.id}`, { headers: _this10.headers }),
					error: null
				};
			} catch (error) {
				if (isAuthError(error)) return {
	...
	_listOAuthClients(params) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				...
				const response = yield _request(_this11.fetch, "GET", `${_this11.url}/admin/oauth/clients`, {
	...
	*/
	_createOAuthClient(params) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this12.fetch, "POST", `${_this12.url}/admin/oauth/clients`, {
					body: params,
	...
	*/
	_getOAuthClient(clientId) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this13.fetch, "GET", `${_this13.url}/admin/oauth/clients/${clientId}`, {
	...
	*/
	_updateOAuthClient(clientId, params) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this14.fetch, "PUT", `${_this14.url}/admin/oauth/clients/${clientId}`, {
					body: params,
	...
	*/
	_deleteOAuthClient(clientId) {
		...
		return _asyncToGenerator(function* () {
			try {
				yield _request(_this15.fetch, "DELETE", `${_this15.url}/admin/oauth/clients/${clientId}`, {
	...
	*/
	_regenerateOAuthClientSecret(clientId) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this16.fetch, "POST", `${_this16.url}/admin/oauth/clients/${clientId}/regenerate_secret`, {
	...
	_listCustomProviders(params) {
		...
		return _asyncToGenerator(function* () {
			try {
				...
				return yield _request(_this17.fetch, "GET", `${_this17.url}/admin/custom-providers`, {
	...
	*/
	_createCustomProvider(params) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this18.fetch, "POST", `${_this18.url}/admin/custom-providers`, {
					body: params,
	...
	*/
	_getCustomProvider(identifier) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this19.fetch, "GET", `${_this19.url}/admin/custom-providers/${identifier}`, {
	...
	*/
	_updateCustomProvider(identifier, params) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _request(_this20.fetch, "PUT", `${_this20.url}/admin/custom-providers/${identifier}`, {
					body: params,
	...
	*/
	_deleteCustomProvider(identifier) {
		...
		return _asyncToGenerator(function* () {
			try {
				yield _request(_this21.fetch, "DELETE", `${_this21.url}/admin/custom-providers/${identifier}`, {
	...
	*/
	_adminListPasskeys(params) {
		var _this22 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this22.experimental);
			validateUUID(params.userId);
			try {
				return yield _request(_this22.fetch, "GET", `${_this22.url}/admin/users/${params.userId}/passkeys`, {
	...
	*/
	_adminDeletePasskey(params) {
		var _this23 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this23.experimental);
			validateUUID(params.userId);
			validateUUID(params.passkeyId);
			try {
				yield _request(_this23.fetch, "DELETE", `${_this23.url}/admin/users/${params.userId}/passkeys/${params.passkeyId}`, {
...
*/
function navigatorLock(_x3, _x4, _x5) {
...
* ```
*/
function processLock(_x6, _x7, _x8) {
...
function _processLock() {
	_processLock = _asyncToGenerator(function* (name, acquireTimeout, fn) {
		...
		const previousOperationHandled = _asyncToGenerator(function* () {
			try {
		...
		const currentOperation = _asyncToGenerator(function* () {
			let timeoutId = null;
			try {
		...
		PROCESS_LOCKS[name] = _asyncToGenerator(function* () {
			try {
...
var WebAuthnApi = class {
	...
	*/
	_enroll(params) {
		var _this = this;
		return _asyncToGenerator(function* () {
			return _this.client.mfa.enroll(Object.assign(Object.assign({}, params), { factorType: "webauthn" }));
		})();
	}
	/**
	...
	*/
	_challenge({ factorId, webauthn, friendlyName, signal }, overrides) {
		var _this2 = this;
		return _asyncToGenerator(function* () {
			var _a;
			try {
				const { data: challengeResponse, error: challengeError } = yield _this2.client.mfa.challenge({
					factorId,
					webauthn
				});
				...
				if (challengeResponse.webauthn.type === "create") {
					...
					if (!user.name) {
						...
						if (!nameToUse) {
							const userData = (yield _this2.client.getUser()).data.user;
				...
				switch (challengeResponse.webauthn.type) {
					...
					case "request": {
						...
						const { data, error } = yield getCredential(Object.assign(Object.assign({}, challengeResponse.webauthn.credential_options), {
							publicKey: options,
							signal: abortSignal
						}));
						if (data) return {
	...
	* */
	_verify({ challengeId, factorId, webauthn }) {
		var _this3 = this;
		return _asyncToGenerator(function* () {
			return _this3.client.mfa.verify({
				factorId,
				challengeId,
				webauthn
			});
	...
	*/
	_authenticate({ factorId, webauthn: { rpId = typeof window !== "undefined" ? window.location.hostname : void 0, rpOrigins = typeof window !== "undefined" ? [window.location.origin] : void 0, signal } = {} }, overrides) {
		var _this4 = this;
		return _asyncToGenerator(function* () {
			if (!rpId) return {
				data: null,
			...
			try {
				...
				const { data: challengeResponse, error: challengeError } = yield _this4.challenge({
					factorId,
					webauthn: {
						rpId,
						rpOrigins
	...
	*/
	_register({ friendlyName, webauthn: { rpId = typeof window !== "undefined" ? window.location.hostname : void 0, rpOrigins = typeof window !== "undefined" ? [window.location.origin] : void 0, signal } = {} }, overrides) {
		var _this5 = this;
		return _asyncToGenerator(function* () {
			if (!rpId) return {
				data: null,
			...
			try {
				...
				const { data: factor, error: enrollError } = yield _this5._enroll({ friendlyName });
				if (!factor) {
					yield _this5.client.mfa.listFactors().then((factors) => {
						var _a;
						return (_a = factors.data) === null || _a === void 0 ? void 0 : _a.all.find((v) => v.factor_type === "webauthn" && v.friendly_name === friendlyName && v.status === "unverified");
					}).then((factor) => factor ? _this5.client.mfa.unenroll({ factorId: factor === null || factor === void 0 ? void 0 : factor.id }) : void 0);
					return {
						data: null,
						error: enrollError
					};
				}
				const { data: challengeResponse, error: challengeError } = yield _this5._challenge({
					factorId: factor.id,
...
var GoTrueClient = class GoTrueClient {
	...
	constructor(options) {
		...
		*/
		...
		if (isBrowser() && globalThis.BroadcastChannel && this.persistSession && this.storageKey) {
			...
			(_c = this.broadcastChannel) === null || _c === void 0 || _c.addEventListener("message", function() {
				var _ref = _asyncToGenerator(function* (event) {
					_this._debug("received broadcast notification from other tab or client", event);
					if (event.data.event === "TOKEN_REFRESHED" || event.data.event === "SIGNED_IN") _this.lastRefreshFailure = null;
					try {
						yield _this._notifyAllSubscribers(event.data.event, event.data.session, false);
					} catch (error) {
						_this._debug("#broadcastChannel", "error", error);
					}
				});
				return function(_x4) {
					return _ref.apply(this, arguments);
	...
	*/
	initialize() {
		...
		return _asyncToGenerator(function* () {
			var _a;
			if (_this2.initializePromise) return yield _this2.initializePromise;
			_this2._pendingInitNotifications = [];
			_this2.initializePromise = _asyncToGenerator(function* () {
				if (_this2.lock != null) return yield _this2._acquireLock(_this2.lockAcquireTimeout, _asyncToGenerator(function* () {
					return yield _this2._initialize();
				}));
				return yield _this2._initialize();
			})();
			const result = yield _this2.initializePromise;
			const queue = (_a = _this2._pendingInitNotifications) !== null && _a !== void 0 ? _a : [];
			_this2._pendingInitNotifications = null;
			for (const n of queue) yield _this2._notifyAllSubscribers(n.event, n.session, n.broadcast);
			return result;
		})();
	}
	/**
		...
		return _asyncToGenerator(function* () {
			...
			try {
				...
				if (isBrowser()) {
					...
					if (_this3._isImplicitGrantCallback(params)) callbackUrlType = "implicit";
					else if (yield _this3._isPKCECallback(params)) callbackUrlType = "pkce";
				}
				/**
				...
				*/
				if (isBrowser() && _this3.detectSessionInUrl && callbackUrlType !== "none") {
					const { data, error } = yield _this3._getSessionFromURL(params, callbackUrlType);
					if (error) {
					...
					setTimeout(_asyncToGenerator(function* () {
						if (redirectType === "recovery") yield _this3._notifyAllSubscribers("PASSWORD_RECOVERY", session);
						else yield _this3._notifyAllSubscribers("SIGNED_IN", session);
					}), 0);
					return { error: null };
				}
				...
				if (isAuthError(error)) return _this3._returnResult({ error });
				return _this3._returnResult({ error: new AuthUnknownError("Unexpected error during initialization", error) });
			} finally {
	...
	*/
	signInAnonymously(credentials) {
		...
		return _asyncToGenerator(function* () {
			var _a, _b, _c;
			try {
				const { data, error } = yield _request(_this4.fetch, "POST", `${_this4.url}/signup`, {
					headers: _this4.headers,
					body: {
				...
				if (error || !data) return _this4._returnResult({
					data: {
						user: null,
						session: null
					},
				...
				if (isAuthError(error)) return _this4._returnResult({
					data: {
						user: null,
						session: null
					},
		...
		return _asyncToGenerator(function* () {
			...
			try {
				...
				if ("email" in credentials) {
					...
					if (_this5.flowType === "pkce") [codeChallenge, codeChallengeMethod, flowId] = yield _this5._getCodeChallengeAndMethod();
					res = yield _request(_this5.fetch, "POST", `${_this5.url}/signup`, {
						headers: _this5.headers,
						redirectTo: _this5._maybeAppendFlowIdToRedirect(options === null || options === void 0 ? void 0 : options.emailRedirectTo, flowId),
						body: {
							email,
							password,
				...
				if (error || !data) {
					yield removePKCEVerifier(_this5.storage, _this5.storageKey, flowId);
					return _this5._returnResult({
						data: {
							user: null,
							session: null
						},
				...
				yield removePKCEVerifier(_this5.storage, _this5.storageKey, flowId);
				if (isAuthError(error)) return _this5._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	signInWithPassword(credentials) {
		...
		return _asyncToGenerator(function* () {
			try {
				...
				const { data, error } = res;
				if (error) return _this6._returnResult({
					data: {
						user: null,
						session: null
					},
				...
				if (isAuthError(error)) return _this6._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	*/
	exchangeCodeForSession(authCode, options) {
		...
		return _asyncToGenerator(function* () {
			...
			if (_this8.lock != null) return _this8._acquireLock(_this8.lockAcquireTimeout, _asyncToGenerator(function* () {
				return _this8._exchangeCodeForSession(authCode, options);
			}));
			return _this8._exchangeCodeForSession(authCode, options);
		})();
	}
	...
	*/
	signInWithWeb3(credentials) {
		...
		return _asyncToGenerator(function* () {
			...
			switch (chain) {
				case "ethereum": return yield _this9.signInWithEthereum(credentials);
				case "solana": return yield _this9.signInWithSolana(credentials);
	...
	signInWithEthereum(credentials) {
		...
		return _asyncToGenerator(function* () {
			...
			if ("message" in credentials) {
				...
				const accounts = yield resolvedWallet.request({ method: "eth_requestAccounts" }).then((accs) => accs).catch(() => {
				...
				if (!chainId) chainId = fromHex(yield resolvedWallet.request({ method: "eth_chainId" }));
			...
			try {
				const { data, error } = yield _request(_this10.fetch, "POST", `${_this10.url}/token?grant_type=web3`, {
				...
				if (isAuthError(error)) return _this10._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	signInWithSolana(credentials) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				const { data, error } = yield _request(_this11.fetch, "POST", `${_this11.url}/token?grant_type=web3`, {
					headers: _this11.headers,
					body: Object.assign({
						chain: "solana",
						message,
				...
				if (data.session) {
					yield _this11._saveSession(data.session);
					yield _this11._notifyAllSubscribers("SIGNED_IN", data.session);
				}
				return _this11._returnResult({
					data: Object.assign({}, data),
					error
				...
				if (isAuthError(error)) return _this11._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	_exchangeCodeForSession(authCode, options) {
		...
		return _asyncToGenerator(function* () {
			...
			if (hasExplicitFlowId && !requestedFlowId) _this12._debug("#_exchangeCodeForSession()", "provided flowId is not a valid flow id", options === null || options === void 0 ? void 0 : options.flowId);
			const { verifier: storageItem, flowId } = hasExplicitFlowId && !requestedFlowId ? {
				verifier: null,
				flowId: null
			} : yield retrievePKCEVerifier(_this12.storage, _this12.storageKey, requestedFlowId);
			const [codeVerifier, redirectType] = (storageItem !== null && storageItem !== void 0 ? storageItem : "").split("/");
			try {
				if (!codeVerifier && _this12.flowType === "pkce") throw new AuthPKCECodeVerifierMissingError();
				const { data, error } = yield _request(_this12.fetch, "POST", `${_this12.url}/token?grant_type=pkce`, {
					headers: _this12.headers,
					body: {
						auth_code: authCode,
				...
				yield removePKCEVerifier(_this12.storage, _this12.storageKey, flowId);
				if (error) throw error;
				if (!data || !data.session || !data.user) {
					const invalidTokenError = new AuthInvalidTokenResponseError();
					return _this12._returnResult({
						data: {
							user: null,
							session: null,
							redirectType: null
				...
				if (data.session) {
					yield _this12._saveSession(data.session);
					yield _this12._notifyAllSubscribers(redirectType === "recovery" ? "PASSWORD_RECOVERY" : "SIGNED_IN", data.session);
				}
				return _this12._returnResult({
				...
				yield removePKCEVerifier(_this12.storage, _this12.storageKey, flowId);
				if (isAuthError(error)) return _this12._returnResult({
					data: {
						user: null,
						session: null,
						redirectType: null
	...
	signInWithIdToken(credentials) {
		...
		return _asyncToGenerator(function* () {
			try {
				...
				const { data, error } = yield _request(_this13.fetch, "POST", `${_this13.url}/token?grant_type=id_token`, {
					headers: _this13.headers,
					body: {
						provider,
						id_token: token,
				...
				if (error) return _this13._returnResult({
					data: {
						user: null,
						session: null
					},
				...
				if (data.session) {
					yield _this13._saveSession(data.session);
					yield _this13._notifyAllSubscribers("SIGNED_IN", data.session);
				}
				return _this13._returnResult({
					data,
					error
				...
				if (isAuthError(error)) return _this13._returnResult({
					data: {
						user: null,
						session: null
					},
		...
		return _asyncToGenerator(function* () {
			...
			try {
				if ("email" in credentials) {
					...
					let codeChallenge = null;
					let codeChallengeMethod = null;
					if (_this14.flowType === "pkce") [codeChallenge, codeChallengeMethod, flowId] = yield _this14._getCodeChallengeAndMethod();
					const { error } = yield _request(_this14.fetch, "POST", `${_this14.url}/otp`, {
						headers: _this14.headers,
						body: {
							email,
							data: (_a = options === null || options === void 0 ? void 0 : options.data) !== null && _a !== void 0 ? _a : {},
						...
						redirectTo: _this14._maybeAppendFlowIdToRedirect(options === null || options === void 0 ? void 0 : options.emailRedirectTo, flowId)
					});
					return _this14._returnResult({
						data: {
							user: null,
							session: null
						},
				...
				if ("phone" in credentials) {
					...
					const { data, error } = yield _request(_this14.fetch, "POST", `${_this14.url}/otp`, {
						headers: _this14.headers,
						body: {
							phone,
				...
				yield removePKCEVerifier(_this14.storage, _this14.storageKey, flowId);
				if (isAuthError(error)) return _this14._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	verifyOtp(params) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				...
				const { data, error } = yield _request(_this15.fetch, "POST", `${_this15.url}/verify`, {
					headers: _this15.headers,
				...
				if (session === null || session === void 0 ? void 0 : session.access_token) {
					yield _this15._saveSession(session);
					yield _this15._notifyAllSubscribers(params.type == "recovery" ? "PASSWORD_RECOVERY" : "SIGNED_IN", session);
				}
				return _this15._returnResult({
					data: {
						user,
						session
					},
				...
				if (isAuthError(error)) return _this15._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	signInWithSSO(params) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				let codeChallenge = null;
				let codeChallengeMethod = null;
				if (_this16.flowType === "pkce") [codeChallenge, codeChallengeMethod, flowId] = yield _this16._getCodeChallengeAndMethod();
				const result = yield _request(_this16.fetch, "POST", `${_this16.url}/sso`, {
					body: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, "providerId" in params ? { provider_id: params.providerId } : null), "domain" in params ? { domain: params.domain } : null), { redirect_to: _this16._maybeAppendFlowIdToRedirect((_a = params.options) === null || _a === void 0 ? void 0 : _a.redirectTo, flowId) }), ((_b = params === null || params === void 0 ? void 0 : params.options) === null || _b === void 0 ? void 0 : _b.captchaToken) ? { gotrue_meta_security: { captcha_token: params.options.captchaToken } } : null), {
				...
				return _this16._returnResult(result);
			} catch (error) {
				yield removePKCEVerifier(_this16.storage, _this16.storageKey, flowId);
				if (isAuthError(error)) return _this16._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	reauthenticate() {
		...
		return _asyncToGenerator(function* () {
			...
			if (_this17.lock != null) return yield _this17._acquireLock(_this17.lockAcquireTimeout, _asyncToGenerator(function* () {
				return yield _this17._reauthenticate();
			}));
			return yield _this17._reauthenticate();
		})();
	}
	_reauthenticate() {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this18._useSession(function() {
					var _ref2 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) throw sessionError;
						if (!session) throw new AuthSessionMissingError();
						const { error } = yield _request(_this18.fetch, "GET", `${_this18.url}/reauthenticate`, {
							headers: _this18.headers,
							jwt: session.access_token
						});
						return _this18._returnResult({
							data: {
								user: null,
								session: null
							},
				...
				if (isAuthError(error)) return _this18._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	*/
	resend(credentials) {
		...
		return _asyncToGenerator(function* () {
			let flowId = null;
			try {
				const endpoint = `${_this19.url}/resend`;
				if ("email" in credentials) {
					const { email, type, options } = credentials;
					let codeChallenge = null;
					let codeChallengeMethod = null;
					if (_this19.flowType === "pkce") [codeChallenge, codeChallengeMethod, flowId] = yield _this19._getCodeChallengeAndMethod();
					const { error } = yield _request(_this19.fetch, "POST", endpoint, {
						headers: _this19.headers,
						body: {
							email,
							type,
						...
						redirectTo: _this19._maybeAppendFlowIdToRedirect(options === null || options === void 0 ? void 0 : options.emailRedirectTo, flowId)
					});
					if (error) yield removePKCEVerifier(_this19.storage, _this19.storageKey, flowId);
					return _this19._returnResult({
						data: {
							user: null,
							session: null
						},
					...
					const { data, error } = yield _request(_this19.fetch, "POST", endpoint, {
						headers: _this19.headers,
						body: {
							phone,
							type,
				...
				yield removePKCEVerifier(_this19.storage, _this19.storageKey, flowId);
				if (isAuthError(error)) return _this19._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	*/
	getSession() {
		...
		return _asyncToGenerator(function* () {
			...
			if (_this20.lock != null) return yield _this20._acquireLock(_this20.lockAcquireTimeout, _asyncToGenerator(function* () {
				return _this20._useSession(function() {
			...
			return yield _this20._useSession(function() {
	...
	*/
	_acquireLock(acquireTimeout, fn) {
		var _this21 = this;
		return _asyncToGenerator(function* () {
			_this21._debug("#_acquireLock", "begin", acquireTimeout);
			try {
				if (_this21.lockAcquired) {
					const last = _this21.pendingInLock.length ? _this21.pendingInLock[_this21.pendingInLock.length - 1] : Promise.resolve();
					const result = _asyncToGenerator(function* () {
						yield last;
						return yield fn();
					})();
					_this21.pendingInLock.push(_asyncToGenerator(function* () {
						try {
							yield result;
						} catch (_e) {}
					})());
				...
				return yield _this21.lock(`lock:${_this21.storageKey}`, acquireTimeout, _asyncToGenerator(function* () {
					_this21._debug("#_acquireLock", "lock acquired for storage key", _this21.storageKey);
					try {
						_this21.lockAcquired = true;
						const result = fn();
						_this21.pendingInLock.push(_asyncToGenerator(function* () {
							try {
								yield result;
						...
						yield result;
						while (_this21.pendingInLock.length) {
							const waitOn = [..._this21.pendingInLock];
							yield Promise.all(waitOn);
							_this21.pendingInLock.splice(0, waitOn.length);
						}
						return yield result;
					} finally {
						_this21._debug("#_acquireLock", "lock released for storage key", _this21.storageKey);
						_this21.lockAcquired = false;
					}
				}));
			} finally {
				_this21._debug("#_acquireLock", "end");
			}
		})();
	}
	/**
	...
	*/
	_useSession(fn) {
		var _this22 = this;
		return _asyncToGenerator(function* () {
			_this22._debug("#_useSession", "begin");
			try {
				return yield fn(yield _this22.__loadSession());
			} finally {
				_this22._debug("#_useSession", "end");
			}
		})();
	}
	/**
	...
	*/
	__loadSession() {
		var _this23 = this;
		return _asyncToGenerator(function* () {
			_this23._debug("#__loadSession()", "begin");
			if (_this23.lock != null && !_this23.lockAcquired) _this23._debug("#__loadSession()", "used outside of an acquired lock!", (/* @__PURE__ */ new Error()).stack);
			try {
				let currentSession = null;
				const maybeSession = yield getItemAsync(_this23.storage, _this23.storageKey);
				_this23._debug("#getSession()", "session from storage", maybeSession);
				if (maybeSession !== null) if (_this23._isValidSession(maybeSession)) currentSession = maybeSession;
				else {
					_this23._debug("#getSession()", "session from storage is not valid");
					yield _this23._removeSession();
				}
				...
				_this23._debug("#__loadSession()", `session has${hasExpired ? "" : " not"} expired`, "expires_at", currentSession.expires_at);
				if (!hasExpired) {
					if (_this23.userStorage) {
						const maybeUser = yield getItemAsync(_this23.userStorage, _this23.storageKey + "-user");
					...
					if (_this23.storage.isServer && currentSession.user && !currentSession.user.__isUserNotAvailableProxy) {
						const suppressWarningRef = { value: _this23.suppressGetSessionWarning };
						currentSession.user = insecureUserWarningProxy(currentSession.user, suppressWarningRef);
						if (suppressWarningRef.value) _this23.suppressGetSessionWarning = true;
					}
					return {
				...
				const { data: session, error } = yield _this23._callRefreshToken(currentSession.refresh_token);
				if (error) {
					if (!!(currentSession.expires_at && currentSession.expires_at * 1e3 > Date.now())) {
						const stillStored = yield getItemAsync(_this23.storage, _this23.storageKey);
						if (stillStored && stillStored.refresh_token === currentSession.refresh_token) return _this23._returnResult({
							data: { session: currentSession },
							error: null
						});
					}
					return _this23._returnResult({
						data: { session: null },
						error
					});
				}
				...
				_this23._debug("#__loadSession()", "end");
			}
		})();
	}
	/**
	...
	*/
	getUser(jwt) {
		var _this24 = this;
		return _asyncToGenerator(function* () {
			if (jwt) return yield _this24._getUser(jwt);
			yield _this24.initializePromise;
			let result;
			if (_this24.lock != null) result = yield _this24._acquireLock(_this24.lockAcquireTimeout, _asyncToGenerator(function* () {
				return yield _this24._getUser();
			}));
			else result = yield _this24._getUser();
			if (result.data.user) _this24.suppressGetSessionWarning = true;
			return result;
		})();
	}
	_getUser(jwt) {
		...
		return _asyncToGenerator(function* () {
			try {
				if (jwt) return yield _request(_this25.fetch, "GET", `${_this25.url}/user`, {
					headers: _this25.headers,
					jwt,
					xform: _userResponse
				});
				return yield _this25._useSession(function() {
					var _ref5 = _asyncToGenerator(function* (result) {
						var _a, _b, _c;
						const { data, error } = result;
						if (error) throw error;
						if (!((_a = data.session) === null || _a === void 0 ? void 0 : _a.access_token) && !_this25.hasCustomAuthorizationHeader) return {
							data: { user: null },
							error: new AuthSessionMissingError()
						};
						return yield _request(_this25.fetch, "GET", `${_this25.url}/user`, {
							headers: _this25.headers,
				...
				if (isAuthError(error)) {
					if (isAuthSessionMissingError(error)) yield _this25._removeSession();
					return _this25._returnResult({
						data: { user: null },
						error
					});
				}
	...
	*/
	updateUser(_x9) {
		...
		return _asyncToGenerator(function* (attributes, options = {}) {
			yield _this26.initializePromise;
			if (_this26.lock != null) return yield _this26._acquireLock(_this26.lockAcquireTimeout, _asyncToGenerator(function* () {
				return yield _this26._updateUser(attributes, options);
			}));
			return yield _this26._updateUser(attributes, options);
		}).apply(this, arguments);
	}
	_updateUser(_x11) {
		...
		return _asyncToGenerator(function* (attributes, options = {}) {
			let flowId = null;
			try {
				return yield _this27._useSession(function() {
					var _ref6 = _asyncToGenerator(function* (result) {
						...
						let codeChallenge = null;
						let codeChallengeMethod = null;
						if (_this27.flowType === "pkce" && attributes.email != null) [codeChallenge, codeChallengeMethod, flowId] = yield _this27._getCodeChallengeAndMethod();
						const { data, error: userError } = yield _request(_this27.fetch, "PUT", `${_this27.url}/user`, {
							headers: _this27.headers,
							redirectTo: _this27._maybeAppendFlowIdToRedirect(options === null || options === void 0 ? void 0 : options.emailRedirectTo, flowId),
						...
						yield _this27._saveSession(session);
						yield _this27._notifyAllSubscribers("USER_UPDATED", session);
						return _this27._returnResult({
				...
				yield removePKCEVerifier(_this27.storage, _this27.storageKey, flowId);
				if (isAuthError(error)) return _this27._returnResult({
					data: { user: null },
					error
				});
				throw error;
	...
	*/
	setSession(currentSession) {
		...
		return _asyncToGenerator(function* () {
			...
			if (_this28.lock != null) return yield _this28._acquireLock(_this28.lockAcquireTimeout, _asyncToGenerator(function* () {
				return yield _this28._setSession(currentSession);
			}));
			return yield _this28._setSession(currentSession);
		})();
	}
	_setSession(currentSession) {
		...
		return _asyncToGenerator(function* () {
			try {
				...
				if (hasExpired) {
					const { data: refreshedSession, error } = yield _this29._callRefreshToken(currentSession.refresh_token);
					if (error) return _this29._returnResult({
						data: {
							user: null,
							session: null
						},
					...
					const { data, error } = yield _this29._getUser(currentSession.access_token);
					if (error) return _this29._returnResult({
						data: {
							user: null,
							session: null
					...
					yield _this29._saveSession(session);
					yield _this29._notifyAllSubscribers("SIGNED_IN", session);
				}
				return _this29._returnResult({
					data: {
						user: session.user,
						session
					},
				...
				if (isAuthError(error)) return _this29._returnResult({
					data: {
						session: null,
						user: null
					},
	...
	*/
	refreshSession(currentSession) {
		...
		return _asyncToGenerator(function* () {
			...
			if (_this30.lock != null) return yield _this30._acquireLock(_this30.lockAcquireTimeout, _asyncToGenerator(function* () {
				return yield _this30._refreshSession(currentSession);
			}));
			return yield _this30._refreshSession(currentSession);
		})();
	}
	_refreshSession(currentSession) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this31._useSession(function() {
					var _ref7 = _asyncToGenerator(function* (result) {
						...
						const { data: session, error } = yield _this31._callRefreshToken(currentSession.refresh_token);
						if (error) return _this31._returnResult({
							data: {
								user: null,
								session: null
							},
						...
						if (!session) return _this31._returnResult({
							data: {
								user: null,
								session: null
							},
				...
				if (isAuthError(error)) return _this31._returnResult({
					data: {
						user: null,
						session: null
					},
		...
		return _asyncToGenerator(function* () {
			...
			try {
				...
				switch (callbackUrlType) {
					case "implicit":
						if (_this32.flowType === "pkce") throw new AuthPKCEGrantCodeExchangeError("Not a valid PKCE flow url.");
						break;
					case "pkce":
						if (_this32.flowType === "implicit") throw new AuthImplicitGrantRedirectError("Not a valid implicit grant flow url.");
						break;
					default:
				}
				if (callbackUrlType === "pkce") {
					_this32._debug("#_initialize()", "begin", "is PKCE flow", true);
					if (!params.code) throw new AuthPKCEGrantCodeExchangeError("No code detected.");
					const { data, error } = yield _this32._exchangeCodeForSession(params.code, { flowId: params[PKCE_FLOW_ID_PARAM] });
					if (error) throw error;
				...
				const { data, error } = yield _this32._getUser(access_token);
				if (error) throw error;
				const session = {
					provider_token,
				...
				_this32._debug("#_getSessionFromURL()", "clearing window.location.hash");
				return _this32._returnResult({
					data: {
						session,
				...
				if (isAuthError(error)) return _this32._returnResult({
					data: {
						session: null,
						redirectType: null
					},
	...
	*/
	_isPKCECallback(params) {
		...
		return _asyncToGenerator(function* () {
			...
			if (flowId && (yield getItemAsync(_this33.storage, pkceVerifierSlotKey(_this33.storageKey, flowId)))) return true;
			return !!(yield getItemAsync(_this33.storage, `${_this33.storageKey}-code-verifier`));
		})();
	}
	/**
	...
	*/
	signOut() {
		var _this34 = this;
		return _asyncToGenerator(function* (options = { scope: "global" }) {
			yield _this34.initializePromise;
			if (_this34.lock != null) return yield _this34._acquireLock(_this34.lockAcquireTimeout, _asyncToGenerator(function* () {
				return yield _this34._signOut(options);
			}));
			return yield _this34._signOut(options);
		}).apply(this, arguments);
	}
	_signOut() {
		var _this35 = this;
		return _asyncToGenerator(function* ({ scope } = { scope: "global" }) {
			return yield _this35._useSession(function() {
				var _ref9 = _asyncToGenerator(function* (result) {
					var _a;
					const removeCurrentSession = function() {
						var _ref8 = _asyncToGenerator(function* () {
							yield _this35._removeSession();
						});
						return function removeCurrentSession() {
							return _ref8.apply(this, arguments);
						};
					}();
					const { data, error: sessionError } = result;
					if (sessionError && !isAuthSessionMissingError(sessionError)) return _this35._returnResult({ error: sessionError });
					const accessToken = (_a = data.session) === null || _a === void 0 ? void 0 : _a.access_token;
					if (accessToken) {
						const { error } = yield _this35.admin.signOut(accessToken, scope);
						if (error) {
							if (!(isAuthApiError(error) && (error.status === 404 || error.status === 401 || error.status === 403) || isAuthSessionMissingError(error))) {
								if (scope !== "others") yield removeCurrentSession();
								return _this35._returnResult({ error });
							}
						}
					}
					if (scope !== "others") yield removeCurrentSession();
					return _this35._returnResult({ error: null });
				});
	...
	*/
	onAuthStateChange(callback) {
		...
		_asyncToGenerator(function* () {
			yield _this36.initializePromise;
			if (_this36.lock != null) yield _this36._acquireLock(_this36.lockAcquireTimeout, _asyncToGenerator(function* () {
				_this36._emitInitialSession(id);
			}));
			else yield _this36._emitInitialSession(id);
		})();
	...
	_emitInitialSession(id) {
		var _this37 = this;
		return _asyncToGenerator(function* () {
			return yield _this37._useSession(function() {
				var _ref10 = _asyncToGenerator(function* (result) {
					var _a, _b;
					try {
						const { data: { session }, error } = result;
						if (error) throw error;
						yield (_a = _this37.stateChangeEmitters.get(id)) === null || _a === void 0 ? void 0 : _a.callback("INITIAL_SESSION", session);
						_this37._debug("INITIAL_SESSION", "callback id", id, "session", session);
					} catch (err) {
						yield (_b = _this37.stateChangeEmitters.get(id)) === null || _b === void 0 ? void 0 : _b.callback("INITIAL_SESSION", null);
						_this37._debug("INITIAL_SESSION", "callback id", id, "error", err);
	...
	* ```
	*/
	resetPasswordForEmail(_x15) {
		var _this38 = this;
		return _asyncToGenerator(function* (email, options = {}) {
			let codeChallenge = null;
			let codeChallengeMethod = null;
			let flowId = null;
			if (_this38.flowType === "pkce") [codeChallenge, codeChallengeMethod, flowId] = yield _this38._getCodeChallengeAndMethod(true);
			try {
				return yield _request(_this38.fetch, "POST", `${_this38.url}/recover`, {
					body: {
						email,
						code_challenge: codeChallenge,
						code_challenge_method: codeChallengeMethod,
					...
					redirectTo: _this38._maybeAppendFlowIdToRedirect(options.redirectTo, flowId)
				});
			} catch (error) {
				yield removePKCEVerifier(_this38.storage, _this38.storageKey, flowId);
				if (isAuthError(error)) return _this38._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	getUserIdentities() {
		...
		return _asyncToGenerator(function* () {
			var _a;
			try {
				const { data, error } = yield _this39.getUser();
				if (error) throw error;
				return _this39._returnResult({
					data: { identities: (_a = data.user.identities) !== null && _a !== void 0 ? _a : [] },
					error: null
				});
			} catch (error) {
				if (isAuthError(error)) return _this39._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	linkIdentity(credentials) {
		var _this40 = this;
		return _asyncToGenerator(function* () {
			if ("token" in credentials) return _this40.linkIdentityIdToken(credentials);
			return _this40.linkIdentityOAuth(credentials);
		})();
	}
	linkIdentityOAuth(credentials) {
		...
		return _asyncToGenerator(function* () {
			...
			let flowId = null;
			try {
				const { data, error } = yield _this41._useSession(function() {
					var _ref11 = _asyncToGenerator(function* (result) {
						var _a, _b, _c, _d, _f;
						const { data, error } = result;
						if (error) throw error;
						const { url, flowId: urlFlowId } = yield _this41._getUrlForProvider(`${_this41.url}/user/identities/authorize`, credentials.provider, {
							redirectTo: (_a = credentials.options) === null || _a === void 0 ? void 0 : _a.redirectTo,
							scopes: (_b = credentials.options) === null || _b === void 0 ? void 0 : _b.scopes,
							queryParams: (_c = credentials.options) === null || _c === void 0 ? void 0 : _c.queryParams,
						...
						flowId = urlFlowId;
						return yield _request(_this41.fetch, "GET", url, {
				...
				if (isAuthError(error)) return _this41._returnResult({
					data: {
						provider: credentials.provider,
						url: null,
						flowId
	...
	linkIdentityIdToken(credentials) {
		var _this42 = this;
		return _asyncToGenerator(function* () {
			return yield _this42._useSession(function() {
				var _ref12 = _asyncToGenerator(function* (result) {
					...
					try {
						...
						if (sessionError) throw sessionError;
						const { options, provider, token, access_token, nonce } = credentials;
						const { data, error } = yield _request(_this42.fetch, "POST", `${_this42.url}/token?grant_type=id_token`, {
							headers: _this42.headers,
						...
						if (error) return _this42._returnResult({
							data: {
								user: null,
								session: null
							},
						...
						else if (!data || !data.session || !data.user) return _this42._returnResult({
							data: {
								user: null,
								session: null
							},
						...
						if (data.session) {
							...
							yield _this42._notifyAllSubscribers("USER_UPDATED", data.session);
						}
						return _this42._returnResult({
							data,
							error
						});
					} catch (error) {
						yield removePKCEVerifier(_this42.storage, _this42.storageKey, null);
						if (isAuthError(error)) return _this42._returnResult({
							data: {
								user: null,
								session: null
							},
	...
	*/
	unlinkIdentity(identity) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this43._useSession(function() {
					var _ref13 = _asyncToGenerator(function* (result) {
						var _a, _b;
						const { data, error } = result;
						if (error) throw error;
						return yield _request(_this43.fetch, "DELETE", `${_this43.url}/user/identities/${identity.identity_id}`, {
							headers: _this43.headers,
				...
				if (isAuthError(error)) return _this43._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_refreshAccessToken(refreshToken) {
		...
		return _asyncToGenerator(function* () {
			...
			_this44._debug(debugName, "begin");
			try {
				const startedAt = Date.now();
				return yield retryable(function() {
					var _ref14 = _asyncToGenerator(function* (attempt) {
						if (attempt > 0) yield sleep(200 * Math.pow(2, attempt - 1));
						_this44._debug(debugName, "refreshing attempt", attempt);
						return yield _request(_this44.fetch, "POST", `${_this44.url}/token?grant_type=refresh_token`, {
							body: { refresh_token: refreshToken },
							headers: _this44.headers,
							xform: _sessionResponse
						});
					});
				...
				_this44._debug(debugName, "error", error);
				if (isAuthError(error)) return _this44._returnResult({
					data: {
						session: null,
						user: null
					},
	...
	_handleProviderSignIn(provider, options) {
		var _this45 = this;
		return _asyncToGenerator(function* () {
			const { url, flowId } = yield _this45._getUrlForProvider(`${_this45.url}/authorize`, provider, {
				redirectTo: options.redirectTo,
				scopes: options.scopes,
				queryParams: options.queryParams
			});
			_this45._debug("#_handleProviderSignIn()", "provider", provider, "options", options, "url", url);
	...
	*/
	_recoverAndRefresh() {
		...
		return _asyncToGenerator(function* () {
			...
			_this46._debug(debugName, "begin");
			try {
				const currentSession = yield getItemAsync(_this46.storage, _this46.storageKey);
				if (currentSession && _this46.userStorage) {
					let maybeUser = yield getItemAsync(_this46.userStorage, _this46.storageKey + "-user");
					if (!_this46.storage.isServer && Object.is(_this46.storage, _this46.userStorage) && !maybeUser) {
						maybeUser = { user: currentSession.user };
						yield setItemAsync(_this46.userStorage, _this46.storageKey + "-user", maybeUser);
					}
					currentSession.user = (_a = maybeUser === null || maybeUser === void 0 ? void 0 : maybeUser.user) !== null && _a !== void 0 ? _a : userNotAvailableProxy();
				} else if (currentSession && !currentSession.user) {
					if (!currentSession.user) {
						const separateUser = yield getItemAsync(_this46.storage, _this46.storageKey + "-user");
						if (separateUser && (separateUser === null || separateUser === void 0 ? void 0 : separateUser.user)) {
							currentSession.user = separateUser.user;
							yield removeItemAsync(_this46.storage, _this46.storageKey + "-user");
							yield setItemAsync(_this46.storage, _this46.storageKey, currentSession);
						} else currentSession.user = userNotAvailableProxy();
					}
				}
				_this46._debug(debugName, "session from storage", currentSession);
				if (!_this46._isValidSession(currentSession)) {
					_this46._debug(debugName, "session is not valid");
					if (currentSession !== null) yield _this46._removeSession();
					return;
				}
				const expiresWithMargin = ((_b = currentSession.expires_at) !== null && _b !== void 0 ? _b : Infinity) * 1e3 - Date.now() < EXPIRY_MARGIN_MS;
				_this46._debug(debugName, `session has${expiresWithMargin ? "" : " not"} expired with margin of ${EXPIRY_MARGIN_MS}s`);
				if (expiresWithMargin) {
					if (_this46.autoRefreshToken && currentSession.refresh_token) {
						const { error } = yield _this46._callRefreshToken(currentSession.refresh_token);
						if (error) if (isAuthRefreshDiscardedError(error)) _this46._debug(debugName, "refresh discarded by commit guard", error);
						else _this46._debug(debugName, "refresh failed", error);
					}
				} else if (currentSession.user && currentSession.user.__isUserNotAvailableProxy === true) try {
					const { data, error: userError } = yield _this46._getUser(currentSession.access_token);
					if (!userError && (data === null || data === void 0 ? void 0 : data.user)) {
						currentSession.user = data.user;
						yield _this46._saveSession(currentSession);
						yield _this46._notifyAllSubscribers("SIGNED_IN", currentSession);
					} else _this46._debug(debugName, "could not get user data, skipping SIGNED_IN notification");
				} catch (getUserError) {
					console.error("Error getting user data:", getUserError);
					_this46._debug(debugName, "error getting user data, skipping SIGNED_IN notification", getUserError);
				}
				else yield _this46._notifyAllSubscribers("SIGNED_IN", currentSession);
			} catch (err) {
				_this46._debug(debugName, "error", err);
				if (isAuthRetryableFetchError(err)) console.warn(err);
				else console.error(err);
				return;
			} finally {
				_this46._debug(debugName, "end");
			}
		})();
	}
	_callRefreshToken(refreshToken) {
		...
		return _asyncToGenerator(function* () {
			...
			if (_this47.refreshingDeferred) return _this47.refreshingDeferred.promise;
			if (_this47.lastRefreshFailure && _this47.lastRefreshFailure.refreshToken === refreshToken && Date.now() < _this47.lastRefreshFailure.expiresAt) {
				_this47._debug("#_callRefreshToken()", "returning cached failure (cooldown active)");
				return _this47.lastRefreshFailure.result;
			}
			const debugName = `#_callRefreshToken()`;
			_this47._debug(debugName, "begin");
			try {
				_this47.refreshingDeferred = new Deferred();
				_this47.refreshingDeferred.promise.then(void 0, () => {});
				const storedAtStart = yield getItemAsync(_this47.storage, _this47.storageKey);
				const { data, error } = yield _this47._refreshAccessToken(refreshToken);
				if (error) throw error;
				if (!data.session) throw new AuthSessionMissingError();
				const storedAfter = yield getItemAsync(_this47.storage, _this47.storageKey);
				if (storedAtStart !== null && (storedAfter === null || storedAfter.refresh_token !== storedAtStart.refresh_token)) {
					_this47._debug(debugName, "commit guard: storage changed since refresh started, discarding rotated tokens", {
						startedWith: "present",
					...
					_this47.refreshingDeferred.resolve(discarded);
					return discarded;
				}
				const epochBeforeSave = _this47._sessionRemovalEpoch;
				yield _this47._saveSession(data.session);
				if (_this47._sessionRemovalEpoch !== epochBeforeSave) {
					_this47._debug(debugName, "commit guard (post-save): _removeSession ran during _saveSession, undoing write");
					yield removeItemAsync(_this47.storage, _this47.storageKey);
					if (_this47.userStorage) yield removeItemAsync(_this47.userStorage, _this47.storageKey + "-user");
					const discarded = {
						data: null,
						error: new AuthRefreshDiscardedError()
					};
				...
				yield _this47._notifyAllSubscribers("TOKEN_REFRESHED", data.session);
				const result = {
					data: data.session,
					error: null
				};
				_this47.lastRefreshFailure = null;
				_this47.refreshingDeferred.resolve(result);
				return result;
			} catch (error) {
				_this47._debug(debugName, "error", error);
				if (isAuthError(error)) {
					const result = {
						data: null,
						error
					...
					if (!isAuthRetryableFetchError(error)) {
						const storedNow = yield getItemAsync(_this47.storage, _this47.storageKey);
						if (!!((storedNow === null || storedNow === void 0 ? void 0 : storedNow.expires_at) && storedNow.expires_at * 1e3 > Date.now())) _this47._debug(debugName, "proactive refresh failed, access token still valid — preserving session");
						else yield _this47._removeSession();
					}
					_this47.lastRefreshFailure = {
						refreshToken,
						result,
						expiresAt: Date.now() + REFRESH_FAILURE_COOLDOWN_MS
					};
					(_a = _this47.refreshingDeferred) === null || _a === void 0 || _a.resolve(result);
					return result;
				}
				(_b = _this47.refreshingDeferred) === null || _b === void 0 || _b.reject(error);
				throw error;
			} finally {
				_this47.refreshingDeferred = null;
				_this47._debug(debugName, "end");
			}
		})();
	}
	_notifyAllSubscribers(event, session, broadcast = true) {
		var _this48 = this;
		return _asyncToGenerator(function* () {
			if (_this48._pendingInitNotifications !== null && broadcast) {
				_this48._pendingInitNotifications.push({
					event,
					session,
					broadcast
				});
			...
			_this48._debug(debugName, "begin", session, `broadcast = ${broadcast}`);
			try {
				if (_this48.broadcastChannel && broadcast) _this48.broadcastChannel.postMessage({
					event,
					session
				});
				const errors = [];
				const promises = Array.from(_this48.stateChangeEmitters.values()).map(function() {
					var _ref15 = _asyncToGenerator(function* (x) {
						try {
	...
	*/
	_saveSession(session) {
		var _this49 = this;
		return _asyncToGenerator(function* () {
			_this49._debug("#_saveSession()", session);
			_this49.suppressGetSessionWarning = true;
			const sessionToProcess = Object.assign({}, session);
			const userIsProxy = sessionToProcess.user && sessionToProcess.user.__isUserNotAvailableProxy === true;
			if (_this49.userStorage) {
				if (!userIsProxy && sessionToProcess.user) yield setItemAsync(_this49.userStorage, _this49.storageKey + "-user", { user: sessionToProcess.user });
				else if (userIsProxy) {}
				const mainSessionData = Object.assign({}, sessionToProcess);
				delete mainSessionData.user;
				const clonedMainSessionData = deepClone(mainSessionData);
				yield setItemAsync(_this49.storage, _this49.storageKey, clonedMainSessionData);
			} else {
				const clonedSession = deepClone(sessionToProcess);
				yield setItemAsync(_this49.storage, _this49.storageKey, clonedSession);
			}
		})();
	}
	_removeSession() {
		var _this50 = this;
		return _asyncToGenerator(function* () {
			_this50._sessionRemovalEpoch += 1;
			_this50._debug("#_removeSession()");
			_this50.lastRefreshFailure = null;
			_this50.suppressGetSessionWarning = false;
			yield removeItemAsync(_this50.storage, _this50.storageKey);
			yield removeAllPKCEVerifiers(_this50.storage, _this50.storageKey);
			yield removeItemAsync(_this50.storage, _this50.storageKey + "-user");
			if (_this50.userStorage) yield removeItemAsync(_this50.userStorage, _this50.storageKey + "-user");
			yield _this50._notifyAllSubscribers("SIGNED_OUT", null);
		})();
	}
	/**
	...
	*/
	_startAutoRefresh() {
		var _this51 = this;
		return _asyncToGenerator(function* () {
			yield _this51._stopAutoRefresh();
			_this51._debug("#_startAutoRefresh()");
			const ticker = setInterval(() => _this51._autoRefreshTokenTick(), AUTO_REFRESH_TICK_DURATION_MS);
			_this51.autoRefreshTicker = ticker;
			...
			const timeout = setTimeout(_asyncToGenerator(function* () {
				yield _this51.initializePromise;
				yield _this51._autoRefreshTokenTick();
			}), 0);
			_this51.autoRefreshTickTimeout = timeout;
	...
	*/
	_stopAutoRefresh() {
		var _this52 = this;
		return _asyncToGenerator(function* () {
			_this52._debug("#_stopAutoRefresh()");
			const ticker = _this52.autoRefreshTicker;
			_this52.autoRefreshTicker = null;
			if (ticker) clearInterval(ticker);
			const timeout = _this52.autoRefreshTickTimeout;
			_this52.autoRefreshTickTimeout = null;
	...
	*/
	startAutoRefresh() {
		var _this53 = this;
		return _asyncToGenerator(function* () {
			_this53._removeVisibilityChangedCallback();
			yield _this53._startAutoRefresh();
		})();
	}
	/**
	...
	*/
	stopAutoRefresh() {
		var _this54 = this;
		return _asyncToGenerator(function* () {
			_this54._removeVisibilityChangedCallback();
			yield _this54._stopAutoRefresh();
		})();
	}
	/**
	...
	*/
	dispose() {
		...
		return _asyncToGenerator(function* () {
			var _a;
			_this55._removeVisibilityChangedCallback();
			yield _this55._stopAutoRefresh();
			(_a = _this55.broadcastChannel) === null || _a === void 0 || _a.close();
			_this55.broadcastChannel = null;
			_this55.stateChangeEmitters.clear();
		})();
	}
	/**
	...
	*/
	_autoRefreshTokenTick() {
		var _this56 = this;
		return _asyncToGenerator(function* () {
			_this56._debug("#_autoRefreshTokenTick()", "begin");
			if (_this56.lock != null) {
				try {
					yield _this56._acquireLock(0, _asyncToGenerator(function* () {
						try {
							const now = Date.now();
							try {
								return yield _this56._useSession(function() {
									var _ref16 = _asyncToGenerator(function* (result) {
										const { data: { session } } = result;
										if (!session || !session.refresh_token || !session.expires_at) {
											_this56._debug("#_autoRefreshTokenTick()", "no session");
											return;
										}
										const expiresInTicks = Math.floor((session.expires_at * 1e3 - now) / AUTO_REFRESH_TICK_DURATION_MS);
										_this56._debug("#_autoRefreshTokenTick()", `access token expires in ${expiresInTicks} ticks, a tick lasts ${AUTO_REFRESH_TICK_DURATION_MS}ms, refresh threshold is 3 ticks`);
										if (expiresInTicks <= 3) yield _this56._callRefreshToken(session.refresh_token);
									});
							...
							_this56._debug("#_autoRefreshTokenTick()", "end");
						}
					}));
				} catch (e) {
					if (e instanceof LockAcquireTimeoutError) _this56._debug("auto refresh token tick lock not available");
					else throw e;
				}
				return;
			}
			if (_this56.refreshingDeferred !== null) {
				_this56._debug("#_autoRefreshTokenTick()", "refresh already in flight, skipping");
				return;
			}
			try {
				const now = Date.now();
				try {
					yield _this56._useSession(function() {
	...
	*/
	_handleVisibilityChange() {
		var _this57 = this;
		return _asyncToGenerator(function* () {
			_this57._debug("#_handleVisibilityChange()");
			if (!isBrowser() || !(window === null || window === void 0 ? void 0 : window.addEventListener)) {
				if (_this57.autoRefreshToken) _this57.startAutoRefresh();
				return false;
			}
			try {
				_this57.visibilityChangedCallback = _asyncToGenerator(function* () {
					try {
						yield _this57._onVisibilityChanged(false);
					} catch (error) {
						_this57._debug("#visibilityChangedCallback", "error", error);
					}
				});
				window === null || window === void 0 || window.addEventListener("visibilitychange", _this57.visibilityChangedCallback);
				yield _this57._onVisibilityChanged(true);
			} catch (error) {
	...
	*/
	_onVisibilityChanged(calledFromInitialize) {
		...
		return _asyncToGenerator(function* () {
			...
			_this58._debug(methodName, "visibilityState", document.visibilityState);
			if (document.visibilityState === "visible") {
				if (_this58.autoRefreshToken) _this58._startAutoRefresh();
				if (!calledFromInitialize) {
					yield _this58.initializePromise;
					if (_this58.lock != null) yield _this58._acquireLock(_this58.lockAcquireTimeout, _asyncToGenerator(function* () {
						if (document.visibilityState !== "visible") {
							_this58._debug(methodName, "acquired the lock to recover the session, but the browser visibilityState is no longer visible, aborting");
							return;
						}
						yield _this58._recoverAndRefresh();
					}));
					else {
						if (document.visibilityState !== "visible") {
							_this58._debug(methodName, "visibilityState is no longer visible, skipping recovery");
							return;
						}
						yield _this58._recoverAndRefresh();
					}
				...
				if (_this58.autoRefreshToken) _this58._stopAutoRefresh();
			}
		})();
	}
	/**
	...
	*/
	_getUrlForProvider(url, provider, options) {
		...
		return _asyncToGenerator(function* () {
			...
			let codeChallengeMethod = null;
			let flowId = null;
			if (_this59.flowType === "pkce") {
				[codeChallenge, codeChallengeMethod, flowId] = yield _this59._getCodeChallengeAndMethod();
				redirectTo = _this59._maybeAppendFlowIdToRedirect(redirectTo, flowId);
			}
	...
	*/
	_getCodeChallengeAndMethod(isPasswordRecovery = false) {
		var _this60 = this;
		return _asyncToGenerator(function* () {
			return getCodeChallengeAndMethod(_this60.storage, _this60.storageKey, isPasswordRecovery, (evictedFlowId) => _this60._debug("#_getCodeChallengeAndMethod()", "evicted oldest pending PKCE verifier slot", evictedFlowId));
		})();
	}
	_unenroll(params) {
		var _this61 = this;
		return _asyncToGenerator(function* () {
			try {
				return yield _this61._useSession(function() {
					var _ref18 = _asyncToGenerator(function* (result) {
						var _a;
						const { data: sessionData, error: sessionError } = result;
						if (sessionError) return _this61._returnResult({
							data: null,
							error: sessionError
						});
						return yield _request(_this61.fetch, "DELETE", `${_this61.url}/factors/${params.factorId}`, {
				...
				if (isAuthError(error)) return _this61._returnResult({
					data: null,
					error
				});
				throw error;
	...
	_enroll(params) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this62._useSession(function() {
					var _ref19 = _asyncToGenerator(function* (result) {
						var _a, _b;
						const { data: sessionData, error: sessionError } = result;
						if (sessionError) return _this62._returnResult({
							data: null,
							error: sessionError
						});
						...
						const { data, error } = yield _request(_this62.fetch, "POST", `${_this62.url}/factors`, {
							body,
							headers: _this62.headers,
							jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
						});
						if (error) return _this62._returnResult({
							data: null,
							error
						});
				...
				if (isAuthError(error)) return _this62._returnResult({
					data: null,
					error
				});
				throw error;
		...
		return _asyncToGenerator(function* () {
			const run = function() {
				var _ref21 = _asyncToGenerator(function* () {
					try {
						return yield _this63._useSession(function() {
							var _ref20 = _asyncToGenerator(function* (result) {
								var _a;
								const { data: sessionData, error: sessionError } = result;
								if (sessionError) return _this63._returnResult({
									data: null,
									error: sessionError
								});
								...
								const { data, error } = yield _request(_this63.fetch, "POST", `${_this63.url}/factors/${params.factorId}/verify`, {
									body,
									headers: _this63.headers,
									jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
								});
								if (error) return _this63._returnResult({
									data: null,
									error
								});
								yield _this63._saveSession(Object.assign({ expires_at: Math.round(Date.now() / 1e3) + data.expires_in }, data));
								yield _this63._notifyAllSubscribers("MFA_CHALLENGE_VERIFIED", data);
								return _this63._returnResult({
									data,
									error
								});
							});
						...
						if (isAuthError(error)) return _this63._returnResult({
							data: null,
							error
						});
						throw error;
			...
			if (_this63.lock != null) return _this63._acquireLock(_this63.lockAcquireTimeout, run);
			return run();
		})();
	}
	_challenge(params) {
		...
		return _asyncToGenerator(function* () {
			const run = function() {
				var _ref23 = _asyncToGenerator(function* () {
					try {
						return yield _this64._useSession(function() {
							var _ref22 = _asyncToGenerator(function* (result) {
								var _a;
								const { data: sessionData, error: sessionError } = result;
								if (sessionError) return _this64._returnResult({
									data: null,
									error: sessionError
								});
								const response = yield _request(_this64.fetch, "POST", `${_this64.url}/factors/${params.factorId}/challenge`, {
									body: params,
									headers: _this64.headers,
									jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
								});
						...
						if (isAuthError(error)) return _this64._returnResult({
							data: null,
							error
						});
						throw error;
			...
			if (_this64.lock != null) return _this64._acquireLock(_this64.lockAcquireTimeout, run);
			return run();
		})();
	}
	/**
	...
	*/
	_challengeAndVerify(params) {
		var _this65 = this;
		return _asyncToGenerator(function* () {
			const { data: challengeData, error: challengeError } = yield _this65._challenge({ factorId: params.factorId });
			if (challengeError) return _this65._returnResult({
				data: null,
				error: challengeError
			});
			return yield _this65._verify({
	...
	*/
	_listFactors() {
		...
		return _asyncToGenerator(function* () {
			var _a;
			const { data: { user }, error: userError } = yield _this66.getUser();
			if (userError) return {
				data: null,
				error: userError
			};
	...
	_getAuthenticatorAssuranceLevel(jwt) {
		...
		return _asyncToGenerator(function* () {
			...
			if (jwt) try {
				...
				const { data: { user }, error: userError } = yield _this67.getUser(jwt);
				if (userError) return _this67._returnResult({
					data: null,
					error: userError
				});
				...
				if (isAuthError(error)) return _this67._returnResult({
					data: null,
					error
				});
				throw error;
			}
			const { data: { session }, error: sessionError } = yield _this67.getSession();
			if (sessionError) return _this67._returnResult({
				data: null,
				error: sessionError
			});
			if (!session) return {
	...
	*/
	_getRecoveryCodesStatus() {
		var _this68 = this;
		return _asyncToGenerator(function* () {
			assertRecoveryCodesExperimentalEnabled(_this68.experimental);
			try {
				return yield _this68._useSession(function() {
					var _ref24 = _asyncToGenerator(function* (result) {
						var _a;
						const { data: sessionData, error: sessionError } = result;
						if (sessionError) return _this68._returnResult({
							data: null,
							error: sessionError
						});
						const { data, error } = yield _request(_this68.fetch, "GET", `${_this68.url}/factors/recovery-codes`, {
							headers: _this68.headers,
							jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
						});
						if (error) return _this68._returnResult({
							data: null,
							error
						});
						return _this68._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this68._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_generateRecoveryCodes(params) {
		var _this69 = this;
		return _asyncToGenerator(function* () {
			assertRecoveryCodesExperimentalEnabled(_this69.experimental);
			try {
				return yield _this69._useSession(function() {
					var _ref25 = _asyncToGenerator(function* (result) {
						var _a;
						const { data: sessionData, error: sessionError } = result;
						if (sessionError) return _this69._returnResult({
							data: null,
							error: sessionError
						});
						const { data, error } = yield _request(_this69.fetch, "POST", `${_this69.url}/factors/recovery-codes`, {
							body: (params === null || params === void 0 ? void 0 : params.friendlyName) ? { friendly_name: params.friendlyName } : void 0,
							headers: _this69.headers,
							jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
						});
						if (error) return _this69._returnResult({
							data: null,
							error
						});
						return _this69._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this69._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_verifyRecoveryCode(params) {
		var _this70 = this;
		return _asyncToGenerator(function* () {
			assertRecoveryCodesExperimentalEnabled(_this70.experimental);
			const run = function() {
				var _ref27 = _asyncToGenerator(function* () {
					try {
						return yield _this70._useSession(function() {
							var _ref26 = _asyncToGenerator(function* (result) {
								var _a;
								const { data: sessionData, error: sessionError } = result;
								if (sessionError) return _this70._returnResult({
									data: null,
									error: sessionError
								});
								const { data, error } = yield _request(_this70.fetch, "POST", `${_this70.url}/factors/recovery-codes/verify`, {
									body: { code: params.code },
									headers: _this70.headers,
									jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
								});
								if (error) return _this70._returnResult({
									data: null,
									error
								});
								const session = Object.assign({ expires_at: expiresAt(data.expires_in) }, data);
								yield _this70._saveSession(session);
								yield _this70._notifyAllSubscribers("MFA_CHALLENGE_VERIFIED", session);
								return _this70._returnResult({
									data,
									error: null
								});
							});
						...
						if (isAuthError(error)) return _this70._returnResult({
							data: null,
							error
						});
						throw error;
			...
			if (_this70.lock != null) return _this70._acquireLock(_this70.lockAcquireTimeout, run);
			return run();
		})();
	}
	/**
	...
	*/
	_regenerateRecoveryCodes() {
		var _this71 = this;
		return _asyncToGenerator(function* () {
			assertRecoveryCodesExperimentalEnabled(_this71.experimental);
			try {
				return yield _this71._useSession(function() {
					var _ref28 = _asyncToGenerator(function* (result) {
						var _a;
						const { data: sessionData, error: sessionError } = result;
						if (sessionError) return _this71._returnResult({
							data: null,
							error: sessionError
						});
						const { data, error } = yield _request(_this71.fetch, "POST", `${_this71.url}/factors/recovery-codes/regenerate`, {
							headers: _this71.headers,
							jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
						});
						if (error) return _this71._returnResult({
							data: null,
							error
						});
						return _this71._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this71._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_unenrollRecoveryCodes() {
		var _this72 = this;
		return _asyncToGenerator(function* () {
			assertRecoveryCodesExperimentalEnabled(_this72.experimental);
			try {
				return yield _this72._useSession(function() {
					var _ref29 = _asyncToGenerator(function* (result) {
						var _a;
						const { data: sessionData, error: sessionError } = result;
						if (sessionError) return _this72._returnResult({
							data: null,
							error: sessionError
						});
						const { data, error } = yield _request(_this72.fetch, "DELETE", `${_this72.url}/factors/recovery-codes`, {
							headers: _this72.headers,
							jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
						});
						if (error) return _this72._returnResult({
							data: null,
							error
						});
						return _this72._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this72._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_getAuthorizationDetails(authorizationId) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this73._useSession(function() {
					var _ref30 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this73._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this73._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						return yield _request(_this73.fetch, "GET", `${_this73.url}/oauth/authorizations/${authorizationId}`, {
							headers: _this73.headers,
							jwt: session.access_token,
							xform: (data) => ({
								data,
								error: null
				...
				if (isAuthError(error)) return _this73._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_approveAuthorization(authorizationId, options) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this74._useSession(function() {
					var _ref31 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this74._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this74._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const response = yield _request(_this74.fetch, "POST", `${_this74.url}/oauth/authorizations/${authorizationId}/consent`, {
							headers: _this74.headers,
							jwt: session.access_token,
							body: { action: "approve" },
							xform: (data) => ({
								data,
				...
				if (isAuthError(error)) return _this74._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_denyAuthorization(authorizationId, options) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this75._useSession(function() {
					var _ref32 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this75._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this75._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const response = yield _request(_this75.fetch, "POST", `${_this75.url}/oauth/authorizations/${authorizationId}/consent`, {
							headers: _this75.headers,
							jwt: session.access_token,
							body: { action: "deny" },
							xform: (data) => ({
								data,
				...
				if (isAuthError(error)) return _this75._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_listOAuthGrants() {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this76._useSession(function() {
					var _ref33 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this76._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this76._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						return yield _request(_this76.fetch, "GET", `${_this76.url}/user/oauth/grants`, {
							headers: _this76.headers,
							jwt: session.access_token,
							xform: (data) => ({
								data,
								error: null
				...
				if (isAuthError(error)) return _this76._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_revokeOAuthGrant(options) {
		...
		return _asyncToGenerator(function* () {
			try {
				return yield _this77._useSession(function() {
					var _ref34 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this77._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this77._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						yield _request(_this77.fetch, "DELETE", `${_this77.url}/user/oauth/grants`, {
							headers: _this77.headers,
							jwt: session.access_token,
				...
				if (isAuthError(error)) return _this77._returnResult({
					data: null,
					error
				});
				throw error;
	...
	fetchJwk(_x37) {
		var _this78 = this;
		return _asyncToGenerator(function* (kid, jwks = { keys: [] }) {
			let jwk = jwks.keys.find((key) => key.kid === kid);
			if (jwk) return jwk;
			const now = Date.now();
			jwk = _this78.jwks.keys.find((key) => key.kid === kid);
			if (jwk && _this78.jwks_cached_at + 6e5 > now) return jwk;
			const { data, error } = yield _request(_this78.fetch, "GET", `${_this78.url}/.well-known/jwks.json`, { headers: _this78.headers });
			if (error) throw error;
			if (!data.keys || data.keys.length === 0) return null;
			_this78.jwks = data;
			_this78.jwks_cached_at = now;
	...
	*/
	getClaims(_x38) {
		var _this79 = this;
		return _asyncToGenerator(function* (jwt, options = {}) {
			try {
				let token = jwt;
				if (!token) {
					const { data, error } = yield _this79.getSession();
					if (error || !data.session) return _this79._returnResult({
						data: null,
						error
					});
				...
				const signingKey = !header.alg || header.alg.startsWith("HS") || !header.kid || !("crypto" in globalThis && "subtle" in globalThis.crypto) ? null : yield _this79.fetchJwk(header.kid, (options === null || options === void 0 ? void 0 : options.keys) ? { keys: options.keys } : options === null || options === void 0 ? void 0 : options.jwks);
				if (!signingKey) {
					const { error } = yield _this79.getUser(token);
					if (error) throw error;
					return {
						data: {
							claims: payload,
				...
				if (!(yield crypto.subtle.verify(algorithm, publicKey, signature, stringToUint8Array(`${rawHeader}.${rawPayload}`)))) throw new AuthInvalidJwtError("Invalid JWT signature");
				return {
					data: {
						claims: payload,
						header,
				...
				if (isAuthError(error)) return _this79._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	signInWithPasskey(credentials) {
		...
		return _asyncToGenerator(function* () {
			var _a, _b, _c;
			assertPasskeyExperimentalEnabled(_this80.experimental);
			try {
				if (!browserSupportsWebAuthn()) return _this80._returnResult({
					data: null,
					error: new AuthUnknownError("Browser does not support WebAuthn", null)
				});
				const { data: options, error: optionsError } = yield _this80._startPasskeyAuthentication({ options: { captchaToken: (_a = credentials === null || credentials === void 0 ? void 0 : credentials.options) === null || _a === void 0 ? void 0 : _a.captchaToken } });
				if (optionsError || !options) return _this80._returnResult({
					data: null,
					error: optionsError
				});
				...
				if (credentialError || !credential) return _this80._returnResult({
					data: null,
				...
				return _this80._verifyPasskeyAuthentication({
					challengeId: options.challenge_id,
					credential: serialized
				});
			} catch (error) {
				if (isAuthError(error)) return _this80._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	registerPasskey(credentials) {
		...
		return _asyncToGenerator(function* () {
			var _a, _b;
			assertPasskeyExperimentalEnabled(_this81.experimental);
			try {
				if (!browserSupportsWebAuthn()) return _this81._returnResult({
					data: null,
					error: new AuthUnknownError("Browser does not support WebAuthn", null)
				});
				const { data: options, error: optionsError } = yield _this81._startPasskeyRegistration();
				if (optionsError || !options) return _this81._returnResult({
					data: null,
					error: optionsError
				});
				...
				if (credentialError || !credential) return _this81._returnResult({
					data: null,
					error: credentialError !== null && credentialError !== void 0 ? credentialError : new AuthUnknownError("WebAuthn ceremony failed", null)
				});
				const serialized = serializeCredentialCreationResponse(credential);
				return _this81._verifyPasskeyRegistration({
					challengeId: options.challenge_id,
					credential: serialized
				});
			} catch (error) {
				if (isAuthError(error)) return _this81._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_startPasskeyRegistration() {
		var _this82 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this82.experimental);
			try {
				return yield _this82._useSession(function() {
					var _ref35 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this82._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this82._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const { data, error } = yield _request(_this82.fetch, "POST", `${_this82.url}/passkeys/registration/options`, {
							headers: _this82.headers,
							jwt: session.access_token,
							body: {}
						});
						if (error) return _this82._returnResult({
							data: null,
							error
						});
						return _this82._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this82._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_verifyPasskeyRegistration(params) {
		var _this83 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this83.experimental);
			try {
				return yield _this83._useSession(function() {
					var _ref36 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this83._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this83._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const { data, error } = yield _request(_this83.fetch, "POST", `${_this83.url}/passkeys/registration/verify`, {
							headers: _this83.headers,
							jwt: session.access_token,
							body: {
						...
						if (error) return _this83._returnResult({
							data: null,
							error
						});
						return _this83._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this83._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_startPasskeyAuthentication(params) {
		...
		return _asyncToGenerator(function* () {
			var _a;
			assertPasskeyExperimentalEnabled(_this84.experimental);
			try {
				const { data, error } = yield _request(_this84.fetch, "POST", `${_this84.url}/passkeys/authentication/options`, {
				...
				if (error) return _this84._returnResult({
					data: null,
					error
				});
				return _this84._returnResult({
					data,
					error: null
				});
			} catch (error) {
				if (isAuthError(error)) return _this84._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_verifyPasskeyAuthentication(params) {
		var _this85 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this85.experimental);
			try {
				const { data, error } = yield _request(_this85.fetch, "POST", `${_this85.url}/passkeys/authentication/verify`, {
					headers: _this85.headers,
					body: {
						challenge_id: params.challengeId,
						credential: params.credential
					},
				...
				if (error) return _this85._returnResult({
					data: null,
					error
				});
				if (data.session) {
					yield _this85._saveSession(data.session);
					yield _this85._notifyAllSubscribers("SIGNED_IN", data.session);
				}
				return _this85._returnResult({
					data,
					error: null
				});
			} catch (error) {
				if (isAuthError(error)) return _this85._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_listPasskeys() {
		var _this86 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this86.experimental);
			try {
				return yield _this86._useSession(function() {
					var _ref37 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this86._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this86._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const { data, error } = yield _request(_this86.fetch, "GET", `${_this86.url}/passkeys`, {
							headers: _this86.headers,
							jwt: session.access_token,
							xform: (data) => ({
								data,
								error: null
						...
						if (error) return _this86._returnResult({
							data: null,
							error
						});
						return _this86._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this86._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_updatePasskey(params) {
		var _this87 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this87.experimental);
			try {
				return yield _this87._useSession(function() {
					var _ref38 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this87._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this87._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const { data, error } = yield _request(_this87.fetch, "PATCH", `${_this87.url}/passkeys/${params.passkeyId}`, {
							headers: _this87.headers,
							jwt: session.access_token,
							body: { friendly_name: params.friendlyName }
						});
						if (error) return _this87._returnResult({
							data: null,
							error
						});
						return _this87._returnResult({
							data,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this87._returnResult({
					data: null,
					error
				});
				throw error;
	...
	*/
	_deletePasskey(params) {
		var _this88 = this;
		return _asyncToGenerator(function* () {
			assertPasskeyExperimentalEnabled(_this88.experimental);
			try {
				return yield _this88._useSession(function() {
					var _ref39 = _asyncToGenerator(function* (result) {
						const { data: { session }, error: sessionError } = result;
						if (sessionError) return _this88._returnResult({
							data: null,
							error: sessionError
						});
						if (!session) return _this88._returnResult({
							data: null,
							error: new AuthSessionMissingError()
						});
						const { error } = yield _request(_this88.fetch, "DELETE", `${_this88.url}/passkeys/${params.passkeyId}`, {
							headers: _this88.headers,
							jwt: session.access_token,
							noResolveJson: true
						});
						if (error) return _this88._returnResult({
							data: null,
							error
						});
						return _this88._returnResult({
							data: null,
							error: null
						});
					});
				...
				if (isAuthError(error)) return _this88._returnResult({
					data: null,
					error
				});
				throw error;
